...
async def init_topic():
    try:
        ...
        if "order_events" not in topic_list:
            logger.info("Топик 'order_events' не найден, создаю...")
            admin.create_topics(
                [NewTopic(name="order_events", num_partitions=1, replication_factor=1)]
            )
            logger.info("Топик создан")
        else:
            logger.info("Топик 'order_events' уже существует")
...
async def produce_kafka_message(
    value: dict, topic: str = "order_events", key: str | None = None
):
    global producer
