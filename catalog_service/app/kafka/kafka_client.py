...
async def init_topic():
    try:
        ...
        if "product_events" not in topic_list:
            logger.info("Топик 'product_events' не найден, создаю...")
            admin.create_topics(
                [
                    NewTopic(
                        name="product_events", num_partitions=1, replication_factor=1
                    )
                ]
            )
            logger.info("Топик создан")
        else:
            logger.info("Топик 'product_events' уже существует")
...
async def produce_kafka_message(
    value: dict, topic: str = "product_events", key: str | None = None
):
    global producer
